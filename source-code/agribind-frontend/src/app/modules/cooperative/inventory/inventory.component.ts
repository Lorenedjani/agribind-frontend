...
export class InventoryComponent implements OnInit {
  ...
  loadSummary(): void {
    this.inventoryService.getSummary().subscribe({
      next: (summary) => {
        // Guard: catchError returns of(undefined) when service is down, which routes through next()
        if (!summary || summary.totalInventoryValue === undefined) {
          console.warn('Inventory summary unavailable (service may be down), using mock data');
