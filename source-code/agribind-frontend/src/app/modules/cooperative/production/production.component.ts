...
export class ProductionComponent implements OnInit {
  ...
  loadAnalytics(): void {
    ...
    if (!this.analyticsStartDate) {
      ...
      this.analyticsStartDate = d.toISOString().split('T')[0];
      this.analyticsEndDate = new Date().toISOString().split('T')[0];
    }
    ...
    const isCoopManager = role === 'COOPERATIVE_MANAGER' || role === 'COOPERATIVE';

    // Government: no cooperativeId filter — backend returns all regions
    // Cooperative: scope by cooperativeId extracted from JWT (not sent from frontend for security,
    // but we include it here as a hint; backend must enforce via JWT regardless)
    const dateRange = `startDate=${this.analyticsStartDate}&endDate=${this.analyticsEndDate}`;
    const scopeParam = isCoopManager && this.user.cooperativeId
      ? `cooperativeId=${this.user.cooperativeId}&`
      : '';

    this.http.get<any>(
      `/api/v1/production/analytics/crops-by-region?${scopeParam}${dateRange}`,
      { headers }
    ).subscribe({
      next: (res: any) => { this.cropsByRegion = res.data || res || []; },
      error: (err: any) => console.error('Failed to load region analytics:', err)
    });

    this.http.get<any>(
      `/api/v1/production/analytics/crops-by-farmer?${scopeParam}${dateRange}`,
      { headers }
    ).subscribe({
      next: (res: any) => { this.cropsByFarmer = res.data || res || []; },
      error: (err: any) => console.error('Failed to load farmer analytics:', err)
    });
  }

  // ────────────── Initializers & Form Methods ──────────────────────────────────────────────────────
