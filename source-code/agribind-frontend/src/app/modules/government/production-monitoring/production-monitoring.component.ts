...
interface CropsByRegion {
  ...
  maize: number;
  total: number;
  /** Distinct crop types in region (from API). */
...
export class ProductionMonitoringComponent implements OnInit, OnDestroy {
  ...
  private readonly apiUrl = '/api/v1/production';

  /** Analytics date range (YYYY-MM-DD); defaults to year-to-date when empty. */
  analyticsStartDate = '';
  analyticsEndDate = '';
  ...
  private applyFilters(): void {
    ...
    this.loadAnalyticsForCrops();
  }

  /** Load crops-by-region and crops-by-farmer from production-monitoring API (stored procedures). */
  private loadAnalyticsForCrops(): void {
    const end = new Date();
    const start = new Date(end.getFullYear(), 0, 1);
    const startStr = this.analyticsStartDate || start.toISOString().split('T')[0];
    const endStr = this.analyticsEndDate || end.toISOString().split('T')[0];
    let regionQ = '';
    if (this.selectedRegion !== 'All Regions') {
      regionQ = `&region=${encodeURIComponent(this.selectedRegion)}`;
    }
    const h = { headers: this.authHeaders() };
    forkJoin({
      byRegion: this.http.get<any>(
        `${this.apiUrl}/analytics/crops-by-region?startDate=${startStr}&endDate=${endStr}`,
        h
      ),
      byFarmer: this.http.get<any>(
        `${this.apiUrl}/analytics/crops-by-farmer?startDate=${startStr}&endDate=${endStr}${regionQ}`,
        h
      )
    }).subscribe({
      next: ({ byRegion, byFarmer }) => {
        const regRows = byRegion?.data ?? byRegion ?? [];
        const farmRows = byFarmer?.data ?? byFarmer ?? [];
        this.buildCropsByRegionFromApi(Array.isArray(regRows) ? regRows : []);
        this.buildCropsByFarmersFromApi(Array.isArray(farmRows) ? farmRows : []);
        this.cdr.markForCheck();
      },
      error: () => {
        this.loadCropsByRegionDataFallback();
        this.loadCropsByFarmersDataFallback();
        this.cdr.markForCheck();
      }
    });
  }

  private mapRegionRow(apiRow: any): CropsByRegion {
    const z = { cocoa: 0, coffee: 0, palmOil: 0, cotton: 0, cassava: 0, maize: 0 };
    const crops = apiRow.crops || [];
    for (const c of crops) {
      const q = Number(c.quantity ?? 0);
      const key = this.bucketCropKey(c.cropName);
      (z as any)[key] += q;
    }
    const total = z.cocoa + z.coffee + z.palmOil + z.cotton + z.cassava + z.maize;
    return {
      name: this.formatRegionLabel(apiRow.region),
      ...z,
      total,
      cropTypeCount: apiRow.cropTypeCount != null ? Number(apiRow.cropTypeCount) : undefined
    };
  }

  private bucketCropKey(cropName: string): keyof Pick<
    CropsByRegion,
    'cocoa' | 'coffee' | 'palmOil' | 'cotton' | 'cassava' | 'maize'
  > {
  ...
  private formatRegionLabel(region: string): string {
    ...
    const s = String(region).replace(/_/g, ' ').toLowerCase();
    return s.replace(/\b\w/g, ch => ch.toUpperCase());
  }

  private buildCropsByRegionFromApi(rows: any[]): void {
    let list = rows.map(r => this.mapRegionRow(r));
    if (this.selectedRegion !== 'All Regions') {
      const sel = this.selectedRegion.toLowerCase();
      list = list.filter(
        r =>
          r.name.toLowerCase() === sel ||
          r.name.toLowerCase().replace(/\s+/g, '') === sel.replace(/\s+/g, '')
      );
    }
    if (this.selectedCrop !== 'All Crops') {
      list = list.map(r => this.zeroOtherCrops(r, this.selectedCrop));
    }
    this.cropsByRegionData = list.sort((a, b) => b.total - a.total);
  }

  private zeroOtherCrops(r: CropsByRegion, crop: string): CropsByRegion {
    const map: Record<string, keyof CropsByRegion> = {
      Cocoa: 'cocoa',
      Coffee: 'coffee',
      'Palm Oil': 'palmOil',
      Cotton: 'cotton',
    ...
    const z = { cocoa: 0, coffee: 0, palmOil: 0, cotton: 0, cassava: 0, maize: 0 };
    (z as any)[keep] = (r as any)[keep];
    const total = z.cocoa + z.coffee + z.palmOil + z.cotton + z.cassava + z.maize;
    return { ...r, ...z, total };
  }

  private buildCropsByFarmersFromApi(rows: any[]): void {
    const byCrop = new Map<string, Map<string, Set<string>>>();
    for (const f of rows) {
      const reg = this.formatRegionLabel(f.farmerRegion || 'Unknown');
      for (const c of f.crops || []) {
        const cropName = c.cropName || 'Unknown';
        if (!byCrop.has(cropName)) {
          byCrop.set(cropName, new Map());
        }
        const rm = byCrop.get(cropName)!;
        if (!rm.has(reg)) {
          rm.set(reg, new Set());
        }
        rm.get(reg)!.add(String(f.farmerId ?? ''));
      }
    }
    ...
    let i = 0;
    const palette = ['#8B4513', '#6B4423', '#228B22', '#E09000', '#B8860B', '#DAA520', '#2d5a8e', '#16a34a'];
    this.cropsByFarmersData = Array.from(byCrop.entries()).map(([cropName, regMap]) => {
      const regions = this.farmerRegions.map(region => {
        const set = regMap.get(region);
        let count = set ? set.size : 0;
      ...
      const totalFarmers = regions.reduce((s, x) => s + x.count, 0);
      return {
        cropName,
        color: cropColors[cropName] || palette[i++ % palette.length],
        totalFarmers,
        regions
      };
    });
    this.cropsByFarmersData = this.cropsByFarmersData
      .filter(c => c.totalFarmers > 0 || this.selectedCrop !== 'All Crops')
      .sort((a, b) => b.totalFarmers - a.totalFarmers);
  }

  private loadCropsByRegionDataFallback(): void {
  ...
  private loadCropsByFarmersDataFallback(): void {
