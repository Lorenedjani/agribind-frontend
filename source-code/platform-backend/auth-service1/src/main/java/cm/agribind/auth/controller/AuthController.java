...
public class AuthController {
    ...
    public ResponseEntity<AuthResponse> qrLogin(@RequestBody QrLoginRequest request) {
        log.info("📱 QR login request received. qrData is {}", request.getQrData() != null ? "present (len=" + request.getQrData().length() + ")" : "NULL");

        if (request.getQrData() == null || request.getQrData().isBlank()) {
            log.error("❌ qrData is null/blank - received body: {}", request);
            return ResponseEntity.badRequest().body(
                AuthResponse.error("QR data is required. Please scan a valid AgriBind QR code.")
            );
        }
    ...
    public ResponseEntity<AuthResponse> sendPhoneOtp(@Valid @RequestBody PhoneOtpSendRequest request) {
        log.info("Phone OTP request received for: {}", request.getPhoneNumber());
        Map<String, Object> response = authService.sendPhoneOtp(request);
        return ResponseEntity.status(HttpStatus.CREATED).body(
                AuthResponse.success("OTP sent successfully", response)
        );
    }

    @PostMapping("/phone/verify-otp")
    public ResponseEntity<AuthResponse> verifyPhoneOtp(@Valid @RequestBody PhoneOtpVerifyRequest request) {
        log.info("Phone OTP verification request received for: {}", request.getPhoneNumber());
        LoginResponse response = authService.verifyPhoneOtp(request);
        return ResponseEntity.ok(
                AuthResponse.success("Phone login successful", response)
        );
    }
