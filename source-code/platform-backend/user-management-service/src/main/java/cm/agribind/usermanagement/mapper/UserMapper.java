...
public interface UserMapper {
    ...
    default void afterUserMapping(@MappingTarget UserResponse response, User user) {
        ...
        if (user instanceof Farmer farmer) {
            ...
            if ((response.getRegion() == null || response.getRegion().isBlank())
                    && cooperative.getOperatingRegion() != null) {
                response.setRegion(cooperative.getOperatingRegion().name());
            }
        } else if (user instanceof CooperativeManager manager) {
            Cooperative coop = manager.getCooperative();
            if (coop != null) {
                response.setCooperativeDetails(CooperativeMapper.INSTANCE.toResponse(coop));
                if ((response.getRegion() == null || response.getRegion().isBlank())
                        && coop.getOperatingRegion() != null) {
                    response.setRegion(coop.getOperatingRegion().name());
                }
            }
