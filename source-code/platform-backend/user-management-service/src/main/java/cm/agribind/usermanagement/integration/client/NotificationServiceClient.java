...
public class NotificationServiceClient {
    ...
    @Value("${agribind.notifications.use-direct-mode:true}")
    ...
    public void sendWelcomeNotification(WelcomeNotificationRequest request) {
        ...
        if (useKafka && kafkaTemplate != null) {
            try {
                ...
                log.warn("⚠️ Kafka failed, falling back to REST: {}", e.getMessage());
            }
        }

        // Use REST API (direct or queued mode)
    ...
    private String buildWelcomeSmsMessage(String name, String username, String password, String userType) {
        ...
        if ("FARMER".equalsIgnoreCase(userType)) {
            return String.format(
                    "Bienvenue sur AgriBind, %s ! Compte agriculteur créé. Identifiant: %s | MDP: %s | Connexion appli: téléphone + OTP ou QR.",
    ...
    /**
     * ✅ REST API for welcome notification
     * Uses /direct endpoint if direct mode is enabled (for dev without RabbitMQ)
     * Uses /welcome endpoint if RabbitMQ is available
     */
    private void sendWelcomeViaRest(WelcomeNotificationRequest request) {
        try {
            // Choose endpoint based on configuration
            String endpoint = useDirectMode ? "/api/notifications/direct/welcome" : "/api/notifications/welcome";
            String url = notificationServiceUrl + endpoint;

            log.info("Sending welcome notification via REST to: {} (Direct Mode: {})", url, useDirectMode);
            ...
            if (response != null && response.isSuccess()) {
                log.info("✅ Welcome notification sent successfully for: {}", request.getUserId());
            } else {
                log.warn("⚠️ Welcome notification sent but response indicates failure for: {}", request.getUserId());
            }
        } catch (Exception e) {
            log.error("❌ Failed to send welcome notification via REST for user: {}", request.getUserId(), e);
            // Don't throw - log and continue to avoid breaking user creation
        }
    }
    ...
    public static class NotificationResponse {
        ...
        private String message;
        private boolean success; // Added for better error handling
