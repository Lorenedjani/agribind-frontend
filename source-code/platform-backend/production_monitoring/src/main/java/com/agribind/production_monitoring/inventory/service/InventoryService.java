package com.agribind.production_monitoring.inventory.service;

import com.agribind.production_monitoring.inventory.dto.InventorySummaryDTO;
import com.agribind.production_monitoring.inventory.model.*;
import com.agribind.production_monitoring.inventory.repository.InventoryRepository;
...
public class InventoryService {
    ...
    private InventoryItem.StockStatus calculateStockStatus(Double quantity, Double minimumQuantity) {
        if (quantity == null || minimumQuantity == null) return InventoryItem.StockStatus.OUT_OF_STOCK;
        if (quantity <= 0) return InventoryItem.StockStatus.OUT_OF_STOCK;
        else if (quantity <= minimumQuantity * 0.3) return InventoryItem.StockStatus.CRITICAL;
        else if (quantity <= minimumQuantity * 0.7) return InventoryItem.StockStatus.LOW_STOCK;
        else return InventoryItem.StockStatus.IN_STOCK;
    }
}
